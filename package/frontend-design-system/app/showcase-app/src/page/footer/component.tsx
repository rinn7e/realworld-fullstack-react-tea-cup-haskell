...
import {  Footer, Hero } from '@rinn7e/realworld-design-system'
...
export const FooterPage: React.FC<Props> = ({ model, dispatch }) => {
  ...
  return (
    <div data-component='FooterPage' className='w-full space-y-8 text-left'>
      {Hero.view({
        variant: 'default',
        size: 'small',
        className:
        ...
        children: () => (
          <>
            ...
            {<TitleMemo size={2} className='mb-2 font-extrabold text-gray-900' children={() => 'Footer'} />}
            <p className='text-base text-gray-600'>
              Page footer container for copyright and links.
            </p>
          </>
        ) })}
      ...
      <div className='flex w-full flex-col gap-6'>
        ...
        {sectionView({
          ...
          children: () => <div className='w-full'>{Footer.view({})}</div> })}

        {model.showCode && (
          <div className='relative w-full overflow-x-auto rounded-lg border border-gray-800 bg-gray-900 p-5 font-mono text-xs text-gray-100 shadow-lg'>
            <div className='mb-3 flex items-center justify-between border-b border-gray-800 pb-3 font-sans text-xs text-gray-400'>
              <span className='font-semibold text-green-400'>JSX / HTML</span>
              <span className='text-gray-500'>Footer Component Code</span>
            </div>
